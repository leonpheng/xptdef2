...
importfiles<-function(...){
  ...
  subDir2<-lst$subfolder[lst$type=="program"]
  ...
  lst<- read.csv("list of files.csv")
  subDir2<-lst$subfolder[lst$type!="program"]
  mainDir2<-paste0(getwd(),"/output/datasets")
  for(i in 1:length(subDir2)){
    dir.create(file.path(mainDir2,subDir2[i]), showWarnings = FALSE)
  }
  ...
  #PMDA
  if(!is.null(unique(lst$subfolder))){
     ...
     to=paste0(odir1,conv[i]),overwrite=T)
      ...
      if(lst$type1[i]=="nonmem"){
        ...
        file.copy(from=sour,to=paste0(odir2,"/",conv[i]),overwrite=T)
          }else(file.copy(from=sour,to=paste0(odir2,"/",conv[i]),overwrite=T))
      }}
    }
...
generateXPT<-function(range.character="yes"){
  ...
  ################## START LOOP #################
original<-paste0(pathdir,"/output/")
...
if(!is.null(unique(lst$subfolder))){
    data<-SASxport::read.xport(paste(sf,"/",inp$outp[j],".xpt",sep=""),name.chars=c("_","-","&","@"))}else{
  ...
  if(!is.null(unique(lst$subfolder))){
    dirr<-sf}else{dirr<-outputdir}

  if(range.character=="no"){
     write.csv(rangepkdat,paste(dirr,"/",inp$outp[j],"define.csv",sep=""),row.names=F)}else{
     write.csv(rgd,paste(dirr,"/",inp$outp[j],"define.csv",sep=""),row.names=F)
      }
...
tabn <- c("Dataset", "Original Name", "Description",
  ...
  if(!is.null(unique(lst$subfolder))){
    hyp110<-gsub(".xpt","",hyp1[i])
    #xpt.location<-paste0(getwd(),"/output/datasets/",lst$subfold[lst$rename==hyp110],"/")
    xpt.location<-paste0("./",lst$subfold[lst$rename==hyp110],"/")
    hyp00<-paste0(lst$subfold[lst$rename==hyp110],"/",hyp1[i])
     }else{xpt.location<-paste0(outputdir,
         paste0(sub(".csv", "", outdir[i]), ".xpt"))
      hyp00<-hyp0}

    loc1 <- paste0(xpt.location, hyp1[i])
    hyp11 <- loc1
    tab[i,5] = pot(hyp00, hyperlink = hyp11, textBold(color = "#0000EE",
                                                         underline = F))
  }
...
if ("logo.png" %in% dir("c:/lhtemplate")) {
  ...
  for (j in 1:nrow(inp)) {
    doc <- addTitle(doc, as.character(tab1data$outp[j]),
                    level = 2)
    if(!is.null(unique(lst$subfolder))){
      #xpt.location<-paste0(getwd(),"/output/datasets/",lst$subfold[lst$rename==tab1data$outp[j]])
      xpt.location<-paste0("./",lst$subfold[lst$rename==tab1data$outp[j]])
      dirr<-paste0(getwd(),"/output/datasets/",lst$subfold[lst$rename==tab1data$outp[j]])}else{xpt.location<-outputdir}
head(lst)

    data <- read.csv(file.path(dirr,paste(inp$outp[j],
                                                "define.csv", sep = "")))
    ...
    if (usedprog[j] == "NA" | usedprog[j] == "") {
      ...
      #prog.location<-paste0(getwd(),"/output/programs/",lst$subfold[lst$rename==usedprog[j]],"/")
      prog.location<-paste0("../programs/",lst$subfold[lst$rename==usedprog[j]],"/")
...
prog <- unlist(paste0(lst$renam[lst$type == "program"],
  ...
  if(!is.null(unique(lst$subfolder))){
    #xpt.location<-paste0(getwd(),"/output/programs/",lst$subfold[j],"/")
    xpt.location<-paste0("./",lst$subfold[j],"/")
    location<-paste0(xpt.location,prog[j])
    }else{
    location <- paste0("#programs#", prog)
    xpt.location<-"./"
    hyp00<-hyp0[i]}
  ...
  if(!is.null(unique(lst$subfolder))){
  define.location="./output/datasets/"}else{define.location=define.location}
...
for (i in 1:length(origprog)) {
  ...
  #xpt.location<-paste0(getwd(),"/output/programs/",ind$subfold[i],"/")
  xpt.location<-paste0("./",ind$subfold[i],"/")
  
  location<-paste0(xpt.location, prog[i])
      op <-location####paste0(prog.location, prog[i])
      space = "\n "
      ...
      io1 <- lst[grep(lst$progNo[lst$filename==lst1],lst$proNo.input), ]
      ...
      io2 <- lst[grep(lst$progNo[lst$filename==lst1],lst$proNo.output), ]
      io3 <- lst[grep(lst$progNo[lst$filename==lst1],lst$progNo.dependent), ]
...
if(ext1!=".xpt"){
 # ext2<-paste0(getwd(),"/output/datasets/",io1$subfold[io],"/")
    ext2<-paste0("./",io1$subfold[io],"/")}else{
      ext2<-paste0("../datasets/",io1$subfold[io],"/")
    }
        #ext2 <- ifelse(io1$type[io] == "program",
           #              prog.location,xpt.location)
      ...
      if (nrow(io2) == 0) {
        ...
        for (io in 1:nrow(io2)) {
          ...
          if(ext1!=".xpt"){
            # ext2<-paste0(getwd(),"/output/datasets/",io1$subfold[io],"/")
            ext2<-paste0("./",io2$subfold[io],"/")}else{
              ext2<-paste0("../datasets/",io2$subfold[io],"/")
            }
          #ext2<-paste0(getwd(),"/output/programs/",io2$subfold[io],"/")
      ...
      if (nrow(io3) == 0) {
        ...
        for (io in 1:nrow(io3)) {
          ...
          if(ext1!=".xpt"){
            # ext2<-paste0(getwd(),"/output/datasets/",io1$subfold[io],"/")
            ext2<-paste0("./",io3$subfold[io],"/")}else{
              ext2<-paste0("../datasets/",io3$subfold[io],"/")
            }
          #ext2<-paste0(getwd(),"/output/programs/",io3$subfold[io],"/")
          ...
          zz2 <- zz2 + pot(txtins, hyperlink = insert, textBold(color = "#0000EE",
                                                   underline = F)) + space +
            pot(orf, textNormal()) + space
...
if(!is.null(unique(lst$subfolder))){
  define.location="./output/programs/"}else{define.location=define.location}
writeDoc(doc, paste0(define.location, "program_procedure.docx"))
...
##CREATE LSV file###
lsdir<-list.dirs(path = ".", full.names = TRUE, recursive = TRUE)
lsdir<-lsdir[!lsdir%in%c(".","./Backup","./input")]
dat1<-NULL
for(i in 1:length(lsdir)){
  dat<-data.frame(
    Folder_file_name=c(lsdir[i],dir(lsdir[i])),
    Hierarchy=c(i,rep(i+1,length(dir(lsdir[i])))),
    Folder=c("x",ifelse(dir(lsdir[i])%in%c(".docx",".xpt",".txt"),"","x")),
    File=c("",ifelse(dir(lsdir[i])%in%c(".docx",".xpt",".txt"),"x","")),
    Original_file_name=c("",rep("please manually complete",length(dir(lsdir[i])))),
    Analysis_Type=c("",rep("please manually complete",length(dir(lsdir[i])))),
    Description=c("",rep("please manually complete",length(dir(lsdir[i]))))
    )
    dat<-dat|>mutate(nchar_Folder_file_name=nchar(Folder_file_name),
                     nchar_Description="Please derive",
                     nchar_Description="Please derive",
                     nchar_File_Path="Please derive")

   dat1<-rbind(dat1,dat)

}
write.csv(dat1,"LSV.csv")
  }else{

  outputdir<-paste0(pathdir,"/output/datasets") # This folder for csv output
  frm<-dir(outputdir)
...
step1<-function(){
  ...
  if(!"list of files.csv"%in%dir()){
    lf<-data.frame(
      ...
      subfolder=c("main","subfold2","xxxx","xxxx","xxxx/yyy","PMDA: must be <=8 character"),
