...
for (j in 1:nrow(inp)){
  ...
  #read.csv.custom(paste(pathwork,definelib,sep="/"))
    ...
    detail$Enter.label.here<-capitalize(as.character(detail$Enter.label.here))
    detail$SAS.label<-capitalize(as.character(detail$Enter.label.here))
    detail$"Detailed.description"[nchar(detail$Variable)>8]<-paste0("\n Note:",detail$"Detailed.description"[nchar(detail$Variable)>8]," Note Original variable name = ",detail$Variable[nchar(detail$Variable)>8],"]")
